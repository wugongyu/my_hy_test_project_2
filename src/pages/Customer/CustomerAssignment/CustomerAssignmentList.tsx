...
import { getCustomersAssignmentList } from "../service";
import { handleFormatTableData } from "@/utils/utils";

export interface CustomerAssignmentListProps{
...
const CustomerAssignmentList: React.FC<CustomerAssignmentListProps> = (props) => {
  ...
  const columns: ProColumns<CustomerContactListItem>[] = [
    ...customerAssignContactTableColumns('customer'),
  ];
  ...
  const fetchList = async(params: CommonTableListParams) => {
    ...
    const res = await getCustomersAssignmentList(currentParams);
  ...
  const headerRender = (
    <>
      <Button
        type="primary"
        onClick={() => handleRecycle('customer')}
      >回收客户</Button>
    </>
  );
  return (
...
export default CustomerAssignmentList;